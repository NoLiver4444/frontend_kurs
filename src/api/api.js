...
const API_BASE = '/.netlify/functions/pandascore-proxy';
...
export async function fetchTournaments(gameSlug = null, limit = 20) {
  try {
    ...
    // ← Запрос через прокси, а не напрямую!
    const url = `${API_BASE}/tournaments?${params.toString()}`;
    console.log('📡 Запрос к API:', url);

    const response = await fetch(url); // ← Без headers (токен на сервере!)
...
export async function fetchTournamentMatches(tournamentId) {
  try {
    ...
    const response = await fetch(url); // ← Без headers!
...
export async function fetchTeamsByGame(gameSlug) {
  try {
    ...
    const tournamentsResponse = await fetch(tournamentsUrl); // ← Без headers!
